...
from .models import Category, addsp, Hoadon
...
def updatecat(id):
    ...
    if request.method =="POST":
        ...
        flash(f'Thay đổi loại thành công','success')
        db.session.commit()
...
def deletecat(id):
    ...
    if request.method=="POST":
        ...
        db.session.commit()
        flash(f'Loại "{deletecat.name}" đã được xoá khỏi hệ thống','success')
        return redirect(url_for('categories'))
    flash(f'Loại "{deletecat.name}" không thể xoá','fail')
    return redirect(url_for('categories'))
...
def addSp():
    ...
    if request.method == "POST":
        ...
        flash(f'{name} đã được thêm vào hệ thống', 'success')
        db.session.commit()
...
def deletesp(id):
    ...
    if request.method=="POST":
        ...
        db.session.commit()
        flash(f'{deletesp.name} đã được xoá khỏi hệ thống','success')
        return redirect(url_for('admin'))
    flash(f'{deletesp.name} không thể xoá','fail')
    return redirect(url_for('admin'))
...
def addCate(): #Thêm loại
    ...
    if request.method=="POST":
        ...
        db.session.add(cate)
        flash(f'Loại {getcate} đã được thêm vào hệ thống', 'success')
        db.session.commit()
        return redirect(url_for('addCate'))
    return render_template('sanpham/addCate.html')

@app.route('/hoadon/<string:taikhoan>/<float:sotien>', methods=['GET', 'POST'])
def hoadon(taikhoan,sotien):   
    hoadon = Hoadon(username = taikhoan, sotien = sotien, ngaymua = datetime.now())
    db.session.add(hoadon)
    db.session.commit()
    session.pop('Shoppingcart', None)
    # flash(f'Mua hàng thành công', 'success')
    return redirect(url_for('home'))
    
@app.route('/lichsu')
def lichsu():
    category = Category.query.join(addsp, (Category.id == addsp.category_id)).all()
    if 'taikhoan' not in session:
        flash(f'Vui lòng đăng nhập trước','danger')
        return redirect(url_for('dangnhap'))
    lichsu = Hoadon.query.order_by(Hoadon.id.desc()).all()
    return render_template('sanpham/lichsu.html', title='Lịch sử',lichsu=lichsu,category=category)